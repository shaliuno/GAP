...
partial class DrawMain {
    ...
    void DrawDebugGui() {
        ...
        var flags = ImGuiWindowFlags.HorizontalScrollbar | ImGuiWindowFlags.NoTitleBar
            | ImGuiWindowFlags.AlwaysVerticalScrollbar;
        ImGui.Begin("Same UI Elements...", flags);
        ui_elem_ptr = ImGui.GetWindowDrawList();
        ImGui.Checkbox("draw children", ref draw_children);

        ImGuiExt.ToolTip("For this option to work you first need to set" +
            "\n the correct window descriptor for the VS you need");

        ImGui.SameLine();
        if (ImGui.Checkbox("this on top", ref ui.sett.b_gui_on_top)) {
            ui.draw_main.SetOverlayTopMost(ui.sett.b_gui_on_top);
            ui.sett.Save();
        }
    ...
    public void AddFrames(Element root) {
        if (frames.Count < max_frames_same_time) {
            ...
            var his = 12;
            ui.AddToLog("selected frame=[" + rect.ToString() + "]");
